...
public class KickCommand implements ICommand {
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        Member selfMember = event.getGuild().getSelfMember(); //Check to see if the bot has permissions
        ...
        event.getGuild().getController().kick(target, String.format("Kick by: %s, with reason: %s", event.getAuthor(), reason)).queue();
